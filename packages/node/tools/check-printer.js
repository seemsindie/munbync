...
try {
  ...
  if (options.includes('--print')) {
    ...
    printer.initialize()
      ...
      .print('Text / CODE128 / QR / PDF417 / Raster\n')
    ...
    printer.print('PDF417 raster: MUNBYNC-OK\n').printPdf417('MUNBYNC-OK').lineFeed();
    if (options.includes('--native-pdf417')) {
      printer.setProfile('generic');
      printer.print('EXPERIMENTAL native PDF417\n').printPdf417Native('MUNBYNC-OK').lineFeed();
      printer.setProfile('itpp047-tested');
    }
    printer.align('center').print('Raster: 128 x 64 checkerboard\n')
      .printRasterImage(0, bitmap, 128, 64).lineFeed()
      .printAndCut('END OF TEST\n');
    console.log('Test receipt sent. Check the complete footer, visible checkerboard, and scan the QR/CODE128/PDF417 symbols.');
  }
